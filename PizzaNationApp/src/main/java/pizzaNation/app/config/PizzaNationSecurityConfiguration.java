...
public class PizzaNationSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.authorizeRequests()
                ...
                .antMatchers(PERMITTED_FOR_ANONYMOUS_ROUTES).access(IS_ANONYMOUS)
                .antMatchers(PERMITTED_FOR_LOGGED_ROUTES).access(IS_AUTHENTICATED)
                .antMatchers(PERMITTED_FOR_ADMIN_AND_MODERATORS_ROUTES).access(HAS_ANY_ROLE_ROLE_ADMIN_ROLE_MODERATOR)
                .antMatchers(PERMITTED_FOR_ADMIN_ROUTES).access(HAS_ANY_ROLE_ROLE_ADMIN)
                .and()
